...
/**
 * Domain type constants for type safety and consistent domain classification
 */
...
/**
 * Domain context type derived from DomainContext constants
 */
export type DomainContext = (typeof DomainContext)[keyof typeof DomainContext];

/**
 * Context score interface for domain detection confidence measurement
 * @interface ContextScore
 */
export interface ContextScore {
  /** The domain being scored */
  domain: DomainContext;
  /** Confidence score (0-1) for this domain */
  score: number;
  /** List of entity names that matched this domain's patterns */
  matchedEntities: string[];
}

/**
 * Result interface for context detection analysis
 * @interface ContextDetectionResult
 */
export interface ContextDetectionResult {
  /** The domain with the highest confidence score */
  primaryContext: DomainContext;
  /** All domain scores sorted by confidence */
  contextScores: ContextScore[];
  /** Map of entity names to their detected domains */
  entityDomainMap: Map<string, DomainContext>;
}

/**
 * ContextDetectionService - Intelligent domain classification for entities
 *
 * This service analyzes entity names and attributes to determine the most appropriate
 * domain context (e-commerce, user management, admin, or generic). It uses pattern
 * matching against predefined keywords and calculates confidence scores for each domain.
 *
 * @class ContextDetectionService
 * @example
 * ```typescript
 * const result = contextDetectionService.detectContext(entities);
 * console.log(result.primaryContext); // 'ecommerce'
 * ```
 */
class ContextDetectionService {
  ...
  detectContext(entities: Entity[]): ContextDetectionResult {
    ...
    for (const entity of entities) {
      ...
      const ecommerceScore = this.calculateDomainScore(
        entityName,
        entityAttributes,
        this.ECOMMERCE_PATTERNS,
      );
      const userMgmtScore = this.calculateDomainScore(
        entityName,
        entityAttributes,
        this.USER_MANAGEMENT_PATTERNS,
      );
      const adminScore = this.calculateDomainScore(
        entityName,
        entityAttributes,
        this.ADMIN_PATTERNS,
      );
  ...
  private determinePrimaryContext(contextScores: ContextScore[]): DomainContext {
    ...
    return secondScore.domain !== DomainContext.GENERIC
      ? secondScore.domain
