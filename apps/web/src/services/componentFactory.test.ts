...
describe('ComponentFactory', () => {
  describe('E-commerce component routing', () => {
    ...
    it('should route order entities to specialized order component', () => {
      ...
      expect(container.querySelector('.border-l-green-500')).not.toBeNull();
      expect(screen.getByText('📦 Order Management')).toBeTruthy();
      expect(componentFactory.hasSpecificComponent(entities[0], contextResult)).toBe(true);
    });

    it('should route customer entities to specialized customer component', () => {
      const entities: Entity[] = [{ name: 'Customer', attributes: ['name', 'email', 'address'] }];
  ...
  describe('User management component routing', () => {
    it('should route user entities to specialized user component', () => {
      const entities: Entity[] = [{ name: 'User', attributes: ['username', 'email', 'password'] }];
      ...
      expect(container.querySelector('.border-l-indigo-500')).not.toBeNull();
      expect(screen.getByText('👤 User Management')).toBeTruthy();
      expect(
        screen.getByText('User account management with authentication and profile settings.'),
      ).toBeTruthy();
      expect(componentFactory.hasSpecificComponent(entities[0], contextResult)).toBe(true);
    });

    it('should route role entities to specialized role component', () => {
      const entities: Entity[] = [{ name: 'Role', attributes: ['name', 'permissions'] }];
  ...
  describe('Admin component routing', () => {
    it('should route system entities to specialized admin component', () => {
      const entities: Entity[] = [{ name: 'System', attributes: ['status', 'version'] }];
      ...
      expect(screen.getAllByText('⚙️ System Admin')[0]).toBeTruthy();
      expect(
        screen.getByText('System administration and configuration management interface.'),
      ).toBeTruthy();
      expect(componentFactory.hasSpecificComponent(entities[0], contextResult)).toBe(true);
    });

    it('should route report entities to specialized report component', () => {
      const entities: Entity[] = [{ name: 'Report', attributes: ['title', 'data'] }];
      ...
      expect(container.querySelector('.border-l-indigo-500')).not.toBeNull();
      expect(screen.getAllByText('📊 Analytics & Reports')[0]).toBeTruthy();
      expect(
        screen.getByText(
  ...
  describe('Generic and fallback behavior', () => {
    it('should fallback to EntityForm for unknown entities', () => {
      const entities: Entity[] = [{ name: 'UnknownEntity', attributes: ['field1', 'field2'] }];
      ...
      expect(screen.getByText('UnknownEntity')).toBeTruthy();
    ...
    it('should fallback to EntityForm for generic domain entities', () => {
      const entities: Entity[] = [{ name: 'Book', attributes: ['title', 'author'] }];
    ...
    it('should handle entities that get detected as specific domain but have no specific component', () => {
      ...
      expect(screen.getByText('SpecialProduct')).toBeTruthy();
      expect(componentFactory.hasSpecificComponent(entities[0], contextResult)).toBe(false);
    });
  });
  ...
  describe('Utility methods', () => {
    it('should return available components for each domain', () => {
      ...
      const userMgmtComponents = componentFactory.getAvailableComponents(
    ...
    it('should allow registering new components', () => {
      ...
      const entities: Entity[] = [{ name: 'CustomEntity', attributes: ['field'] }];
