...
describe('ContextDetectionService', () => {
  ...
  describe('Generic context detection', () => {
    ...
    it('should default to generic when no clear context emerges', () => {
      const entities: Entity[] = [{ name: 'RandomEntity', attributes: ['field1', 'field2'] }];
  ...
  describe('Mixed context scenarios', () => {
    ...
    it('should handle edge case with equal scores', () => {
      ...
      expect([DomainContext.ECOMMERCE, DomainContext.USER_MANAGEMENT]).toContain(
  ...
  describe('Utility methods', () => {
    it('should correctly identify specific vs generic contexts', () => {
      const ecommerceEntities: Entity[] = [{ name: 'Product', attributes: ['price'] }];
      const genericEntities: Entity[] = [{ name: 'Something', attributes: ['field'] }];
    ...
    it('should return correct entity domain from context result', () => {
      ...
      expect(contextDetectionService.getEntityDomain('Product', result)).toBe(
        DomainContext.ECOMMERCE,
      );
      expect(contextDetectionService.getEntityDomain('Unknown', result)).toBe(
        DomainContext.GENERIC,
      );
      expect(contextDetectionService.getEntityDomain('NonExistent', result)).toBe(
        DomainContext.GENERIC,
      );
    });
  });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle entities with empty attributes', () => {
      const entities: Entity[] = [{ name: 'Product', attributes: [] }];
    ...
    it('should handle case-insensitive matching', () => {
      const entities: Entity[] = [{ name: 'PRODUCT', attributes: ['PRICE', 'NAME'] }];
