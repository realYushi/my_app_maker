  ...
  const fallback = (
    <div className="m-4 rounded-lg border border-yellow-200 bg-yellow-50 p-6">
      <div className="mb-4 flex items-center">
        ...
        <div className="ml-3">
          <h3 className="text-sm font-medium text-yellow-800">Generation Component Error</h3>
        </div>
      </div>
      <p className="mb-4 text-sm text-yellow-700">
        There was an issue displaying the generated app content. You can try generating a new app or
        refresh the page.
      </p>
      <button
        onClick={() => window.location.reload()}
        className="rounded bg-yellow-100 px-3 py-2 text-sm font-medium text-yellow-800 transition-colors hover:bg-yellow-200"
      >
        Refresh Page
      </button>
    </div>
  ...
  const fallback = (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
      <div className="text-center">
        ...
        <p className="text-sm text-gray-600">Unable to display this form component</p>
      </div>
    </div>
  );
