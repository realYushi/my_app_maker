...
const getInputType = (attribute: string): string => {
  ...
  if (
    lowercaseAttr.includes('date') ||
    lowercaseAttr.includes('created') ||
    lowercaseAttr.includes('updated')
  )
    return 'date';
  if (
    lowercaseAttr.includes('age') ||
    lowercaseAttr.includes('count') ||
    lowercaseAttr.includes('number')
  )
    return 'number';
  if (
    lowercaseAttr.includes('url') ||
    lowercaseAttr.includes('link') ||
    lowercaseAttr.includes('website')
  )
    return 'url';
  return 'text';
};
...
const EntityForm = memo(({ entity, tabKey = undefined }: EntityFormProps) => {
  ...
  const categorizedFields = useMemo(() => {
    const essential = fieldData.filter(
      field =>
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                viewMode === 'detailed'
      ...
      <div className="p-4 sm:p-6">
        {viewMode === 'summary' ? (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {categorizedFields.essential.length > 0 ? (
                ...
                <div className="col-span-2 py-6 text-center text-gray-500">
                  <TableCellsIcon className="mx-auto mb-2 h-8 w-8 text-gray-300" />
                  <p className="text-sm">
                    No essential fields detected. Click &quot;Show Details&quot; to view all fields.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="pt-4">
              <button
                type="button"
                className="w-full rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                disabled
              >
            ...
            {({ open }) => (
              <div className="space-y-4">
                          ...
                          <div key={index} className="space-y-1">
                            ...
                            field.name.toLowerCase().includes('notes') ? (
                              <textarea
                                ...
                                rows={3}
                                className="w-full resize-none rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                placeholder={field.placeholder}
                                disabled
                              />
                ...
                <div className="flex flex-col gap-3 border-t border-gray-100 pt-4 sm:flex-row">
                  ...
                  <button
                    type="button"
                    className="rounded-md bg-gray-100 px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                    disabled
                  >
                    Cancel
                  </button>
        ...
        <div className="mt-6 border-t border-gray-100 pt-4">
          <p className="text-xs italic text-gray-500">
            ...
            {viewMode === 'summary'
              ? ' Switch to detailed view for all fields.'
              : ' Switch to summary view for essential fields only.'}
          </p>
        </div>
      </div>
    </div>
