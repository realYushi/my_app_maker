...
describe('EntityForm', () => {
  ...
  it('has responsive design classes', () => {
    ...
    expect(formContainer).toHaveClass('bg-white', 'border', 'border-gray-200', 'rounded-lg', 'overflow-hidden', 'hover:shadow-md')
  })
  ...
  describe('Progressive Entity Disclosure (Task 2)', () => {
    const entityWithManyFields: Entity = {
      name: 'ComplexUser',
      attributes: [
        'id', 'name', 'title', // essential fields (id, name, title)
        'email', 'description', 'age', 'category', 'status', 'notes' // detailed fields
      ]
    }
    ...
    it('renders entity header with view mode toggle', () => {
      ...
      expect(screen.getByRole('button', { name: /show details|show summary/i })).toBeInTheDocument()
    })
    ...
    it('updates form footer message based on view mode', async () => {
      ...
      expect(screen.getByText(/switch to summary view for essential fields only/i)).toBeInTheDocument()
    })
